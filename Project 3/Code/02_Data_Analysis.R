...
fem_health_surv_dat <- data.frame(
  ...
  surv_est = paste0(round(c((1-summary(fem_health_surv, times = 10*365.25)$surv)*100),2), "%")
)
...
fem_high_sbp_surv_dat <- data.frame(
  ...
  surv_est = paste0(round(c((1-summary(fem_high_sbp_surv, times = 10*365.25)$surv)*100), 2), "%")
)
...
fem_diab_surv_dat <- data.frame(
  ...
  surv_est = paste0(round(c((1-summary(fem_diab_surv, times = 10*365.25)$surv)*100), 2), "%")
)
...
fem_high_diab_surv_dat <- data.frame(
  ...
  surv_est = paste0(round(c((1-summary(fem_high_diab_surv, times = 10*365.25)$surv)*100), 2), "%")
)
...
fem_smoke_surv_dat <- data.frame(
  ...
  surv_est = paste0(round(c((1-summary(fem_smoke_surv, times = 10*365.25)$surv)*100), 2), "%")
)
...
male_health_surv_dat <- data.frame(
  age = ages,
  scenario = rep("Healthy", 3),
  surv_est = paste0(round(c((1-summary(male_health_surv, times = 10*365.25)$surv)*100), 2), "%")
)
...
male_high_sbp_surv_dat <- data.frame(
  age = ages,
  scenario = rep("High Systolic Blood Pressure", 3),
  surv_est = paste0(round(c((1-summary(male_high_sbp_surv, times = 10*365.25)$surv)*100), 2), "%")
)
...
male_diab_surv_dat <- data.frame(
  age = ages,
  scenario = rep("Diabetic", 3),
  surv_est = paste0(round(c((1-summary(male_diab_surv, times = 10*365.25)$surv)*100), 2), "%")
)
...
male_high_diab_surv_dat <- data.frame(
  age = ages,
  scenario = rep("High Systolic Blood Pressure and Diabetic", 3),
  surv_est = paste0(round(c((1-summary(male_high_diab_surv, times = 10*365.25)$surv)*100), 2), "%")
)
...
male_smoke_surv_dat <- data.frame(
  age = ages,
  scenario = rep("Smoker", 3),
  surv_est = paste0(round(c((1-summary(male_smoke_surv, times = 10*365.25)$surv)*100), 2), "%")
)
...
surv_wide %>%
  kable(digits = 2,
        ...
        align = 'lccc',
        caption = "<b>Table 3.</b> Estimated 10-year stroke risks by risk profile, age, and sex in the Framingham Heart Study",
        escape = F) %>%
  kable_classic(full_width = F, html_font = "Cambria") %>%
  kable_styling(bootstrap_options = "condensed") %>%
